...
use super::aead::{
    Aead, Aes256Ctr, MLKEM768_CT_LEN, MLKEM768_EK_LEN, Padding, TAG_LEN, X25519_LEN,
    blake3_sum256, create_padding, decode_header, decode_length, encode_header,
    encode_length, new_ctr, parse_padding,
};
use ctr_vless::cipher::StreamCipher;
...
enum NfsKey {
    X25519(PublicKey),
    /// Boxed: an ML-KEM-768 encapsulation key is ~1.2 KiB (avoids a large
    /// enum).
    ...
    pub fn init(
        ...
        for k in &nfs_keys_bytes {
            if k.len() == X25519_LEN {
                ...
                let ek = EncapsulationKey::<MlKem768>::new_from_slice(k).map_err(
                    |_| "invalid ML-KEM-768 encapsulation key".to_string(),
                )?;
    ...
    pub async fn handshake(&self, mut stream: AnyStream) -> io::Result<AnyStream> {
        ...
        for (j, key) in self.nfs_keys.iter().enumerate() {
            let index = match key {
                ...
                NfsKey::MlKem(ek) => {
                    ...
                    relays[pos..pos + MLKEM768_CT_LEN]
                        .copy_from_slice(ct.as_slice());
        ...
        if self.seconds > 0 {
            ...
            if let Some((pfs_key, ticket)) = cached {
                ...
                let write_ctr =
                    (self.xor_mode == 2).then(|| new_ctr(&united_key, &iv));
        ...
        client_hello.extend_from_slice(
            &nfs_aead.seal(&encode_length(pfs_key_exchange_len - 18)),
        );
        ...
        client_hello
            .extend_from_slice(&nfs_aead.seal(&encode_length(padding_length - 18)));
        ...
        for (i, &l) in padding_lens.iter().enumerate() {
            if l > 0 {
                ...
                stream.write_all(&rest[..take]).await?;
                rest = &rest[take..];
            }
        ...
        stream.read_exact(&mut enc_pfs_pub).await?;
        let dec = nfs_aead
        ...
        stream.read_exact(&mut enc_length).await?;
        ...
        let (write_ctr, read_ctr) = if self.xor_mode == 2 {
            let ticket_iv: [u8; 16] =
                ticket[..16].try_into().expect("16-byte ticket");
            (
...
pub(super) fn spawn_relay(stream: AnyStream, dp: DataPhase) -> AnyStream {
...
/// (Interop-safe either way: the server flips its cipher if the guess is
/// wrong.)
