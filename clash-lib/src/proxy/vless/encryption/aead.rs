...
fn derive_key(ctx: &[u8], key_material: &[u8]) -> [u8; 32] {
    super::raw_blake3::derive_key(ctx, key_material)
}
...
impl Aead {
    ...
    fn decrypt(
        ...
        ct: &[u8],
        ad: &[u8],
    ) -> Result<Vec<u8>, aes_gcm_vless::Error> {
    ...
    /// Seal with the auto-incrementing counter and associated data (record
    /// header).
    ...
    /// Open with the auto-incrementing counter and associated data (record
    /// header).
    pub(crate) fn open_ad(
        &mut self,
        ct: &[u8],
        ad: &[u8],
    ) -> Result<Vec<u8>, aes_gcm_vless::Error> {
