//! Reference **server** port of Xray/mihomo `encryption/server.go`, used only
//! by the loopback integration test to verify the client against an independent
...
use super::{
    aead::{
        Aead, MLKEM768_CT_LEN, MLKEM768_EK_LEN, Padding, X25519_LEN, blake3_sum256,
        create_padding, decode_length, encode_length, new_ctr, parse_padding,
    },
    client::{DataPhase, spawn_relay},
};
...
impl ServerInstance {
    ...
    pub(crate) async fn handshake(
        ...
        for (j, key) in self.keys.iter().enumerate() {
            ...
            match key {
                ServerKey::X25519(sk, _) => {
                    let peer: [u8; 32] =
                        relays[pos..pos + X25519_LEN].try_into().unwrap();
                    if peer[31] > 127 {
        ...
        let client_ek = EncapsulationKey::<MlKem768>::new_from_slice(
            &client_pfs[..MLKEM768_EK_LEN],
        )
        .map_err(|_| server_err("bad client ML-KEM key"))?;
        let (ct, mlkem_shared) = client_ek.encapsulate();
        let peer_x: [u8; 32] = client_pfs
            [MLKEM768_EK_LEN..MLKEM768_EK_LEN + X25519_LEN]
            .try_into()
            .unwrap();
        ...
        server_hello
            .extend_from_slice(&aead.seal(&encode_length(padding_length - 18)));
        server_hello
            .extend_from_slice(&aead.seal(&vec![0u8; padding_length - 18 - 16]));
        ...
        stream.read_exact(&mut enc_len2).await?;
        let dec2 = nfs_aead
            .open(&enc_len2)
...
fn server_err(msg: &str) -> io::Error {
    io::Error::new(
        ...
        format!("vless-encryption(server): {msg}"),
    )
}
