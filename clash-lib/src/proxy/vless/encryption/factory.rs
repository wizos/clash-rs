...
use super::{
    aead::{MLKEM768_EK_LEN, X25519_LEN},
...
/// - anything else → `Err(_)`.
pub fn parse_client_encryption(
    encryption: &str,
) -> Result<Option<ClientInstance>, String> {
    match encryption {
        "" | "none" => return Ok(None),
        _ => {}
    }
...
mod tests {
    ...
    fn rejects_non_mlkem() {
        ...
        assert!(
            parse_client_encryption("mlkem768x25519plus.bogus.1rtt.KEY").is_err()
        );
    }
    ...
    fn accepts_issue_301_key_with_noncanonical_trailing_bits() {
        ...
        // must too.
        let enc = "mlkem768x25519plus.native.0rtt.\
                   DA7B2WRj7X2zGFwMelbIbcaoUrpLjzoPpmydYW8NvQW";
