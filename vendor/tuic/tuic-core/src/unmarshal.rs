    ...
    pub async fn async_unmarshal(
        ...
        match cmd {
            ...
            Header::TYPE_CODE_CONNECT => {
                Connect::async_read(s).await.map(Self::Connect)
            }
            Header::TYPE_CODE_PACKET => {
                Packet::async_read(s).await.map(Self::Packet)
            }
            Header::TYPE_CODE_DISSOCIATE => {
                Dissociate::async_read(s).await.map(Self::Dissociate)
            }
            Header::TYPE_CODE_HEARTBEAT => {
                Heartbeat::async_read(s).await.map(Self::Heartbeat)
            }
