...
use crate::{
    ...
    Dissociate as DissociateHeader, Heartbeat as HeartbeatHeader,
    ...
    B: AsRef<[u8]>,
{
    ...
    pub fn recv_packet_unrestricted(
        &self,
        header: PacketHeader,
    ) -> Packet<side::Rx, B> {
        let (assoc_id, pkt_id, frag_total, frag_id, size, addr) = header.into();
    ...
    B: AsRef<[u8]>,
{
    ...
    fn recv_packet(
        ...
        self.sessions.get_mut(&assoc_id).map(|session| {
            session.recv_packet(
                sessions, assoc_id, pkt_id, frag_total, frag_id, size, addr,
            )
        })
    }
